...
let typewriter = () => {
  document.querySelector(".centered").innerHTML = messageArray.substring(
    0,
    textPosition
  );
  // console.log(textPosition,messageArray.length,messageArray[0].substring(0,2) );
  if (textPosition === messageArray.length)
    document.querySelector(".welcome").innerHTML = messageArray.substring(
      0,
      textPosition
    );
  if (textPosition++ != messageArray.length) setTimeout(typewriter, speed);
};
...
document.getElementById("countdown").onscroll = function () {
  myFunction();
};
...
function myFunction() {
  ...
  document.getElementById("welcomee-text").style.visibility = "visible";
  document.getElementById("welcomee-text").style.opacity = 1;
  document.getElementById("welcomee-text").style.transition =
    "opacity 2s linear";
